...
class CybertronEngine:
    ...
    def configure_pipeline(self, model_name):
        ...
        # Détection sémantique ultra-flexible
        spacy_code = "fr_core_news_md" if "md" in str(model_name).lower() else "fr_core_news_sm"
    ...
    def predict(self, text, model_name):
        # Interception prioritaire pour la simulation Gemini
        if "Gemini" in str(model_name):
            time.sleep(0.8)
            return {
                ...
                "method": "Gemini Pro Generative API (Simulation)"
            }
            
        # Initialisation du pipeline
        self.configure_pipeline(model_name)
        
        # Traitement du texte et extraction des entités suspectes
        if self.nlp is None:
            try:
                self.nlp = spacy.load("fr_core_news_sm")
            except OSError:
                return {
                    "response": "Erreur système : Impossible de charger le moteur de traitement linguistique.",
                    "threat_level": "🔴 ERREUR CRITIQUE",
                    "tokens": [],
